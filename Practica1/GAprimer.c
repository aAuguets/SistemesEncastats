...
static uint32_t s_1[8]; //s-1
static uint32_t s_2[8]; //s-2
static uint8_t b[8] = {436,419,400,380,298,258,202,143}; //calculats amb l'octave. octave_b.png
static volatile int n = 0; //comptador per saber quin calcul de goertzel s'ha de fer.
static uint32_t potencies[8];
...
static void calcula_potencia(void){
  for(int i=0; i<8; i++){
    if(((int32_t)s_1[i]*s_1[i] + (int32_t)s_2[i]*s_2[i] - (((int32_t)b[i]*s_1[i])>>8) * s_2[i])>10000){
      printf("OK.");
    }
  }
}
